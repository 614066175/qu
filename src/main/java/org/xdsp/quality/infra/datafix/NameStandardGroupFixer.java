...
import static org.xdsp.quality.infra.constant.StandardConstant.StandardType.NAME;
 ...
 * @date: 2023/2/3 11:03
 */
@Component(value = "NAME_STANDARD_FIXER")
public class NameStandardGroupFixer implements GroupDataFixer {
    ...
    public NameStandardGroupFixer(StandardGroupRepository standardGroupRepository, CommonGroupRepository commonGroupRepository) {
    ...
    public void groupDataFix() {
        ...
        //查询所有数据标准分组
        List<StandardGroup> groups = standardGroupRepository.select(StandardGroup.builder().standardType(NAME).build());
        ...
        //修复分组数据
        List<NameStandard> nameStandards = nameStandardRepository.selectAll();
        if (CollectionUtils.isNotEmpty(nameStandards)) {
            nameStandards.forEach(nameStandard -> {
                nameStandard.setGroupId(groupMap.getOrDefault(nameStandard.getGroupId(), 0L));
            });
            nameStandardRepository.batchUpdateOptional(nameStandards, CommonGroup.FIELD_GROUP_ID);
        }
    }
    ...
    private void doFix(StandardGroup group, Map<Long, StandardGroup> standardGroupMap, Map<Long, Long> groupMap, Set<Long> fixedGroup) {
        ...
        //构建新分组数据
        CommonGroup commonGroup = CommonGroup.builder().groupType(NAME_STANDARD)
