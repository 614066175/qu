...
import org.xdsp.quality.infra.constant.AlertTemplate;
import org.xdsp.quality.infra.constant.CheckConstants;
import org.xdsp.quality.infra.constant.ErrorCode;
import org.xdsp.quality.infra.constant.PlanConstant;
import org.xdsp.quality.infra.constant.PlanConstant.CompareSymbol;
import org.xdsp.quality.infra.constant.PlanConstant.ExceptionParam;
...
import org.xdsp.quality.infra.dataobject.SpecifiedParamsResponseDO;
...
import org.xdsp.quality.infra.measure.MeasureCollector;
import org.xdsp.quality.infra.publisher.QualityNoticePublisher;
import org.xdsp.quality.infra.util.CustomThreadPool;
import org.xdsp.quality.infra.util.JsonUtils;
import org.xdsp.quality.infra.util.ParamsUtil;
            ...
            Future<BatchResultBase> future = executor.submit(() ->
            {
                ...
                if (DbType.hive.equals(driverSession.getDbType())) {
                    maps = driverSession.executeOneQuery(batchPlanBase.getDatasourceSchema(),
                            String.format("select count(*) as COUNT from (%s) %s limit 1", batchPlanBase.getObjectName(), StringUtils.isNotEmpty(batchPlanBase.getWhereCondition()) ? String.format("where %s", batchPlanBase.getWhereCondition()) : ""));
                } else {
                    maps = driverSession.executeOneQuery(batchPlanBase.getDatasourceSchema(),
                            String.format("select count(*) as COUNT from (%s) %s", batchPlanBase.getObjectName(), StringUtils.isNotEmpty(batchPlanBase.getWhereCondition()) ? String.format("where %s", batchPlanBase.getWhereCondition()) : ""));
                }
