 ...
 */
@CheckItem("FIELD_UNIQUE_FIXED_VALUE")
public class UniqueFixedValueMeasure implements Measure {

    private static final String SQL = "SELECT COUNT(*) actualValue FROM (SELECT %s FROM %s GROUP BY %s HAVING COUNT(*) = 1) a";
    private final DatasourceFeign datasourceFeign;

    public UniqueFixedValueMeasure(DatasourceFeign datasourceFeign) {
