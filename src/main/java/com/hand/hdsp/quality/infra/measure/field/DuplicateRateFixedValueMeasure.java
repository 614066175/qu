 ...
 */
@CheckItem("FIELD_DUPLICATE_RATE_FIXED_VALUE")
public class DuplicateRateFixedValueMeasure implements Measure {

    private static final String SQL = "SELECT COUNT(*) actualValue FROM (SELECT %s FROM %s GROUP BY %s HAVING COUNT(*) != 1) a";
    private final DatasourceFeign datasourceFeign;

    public DuplicateRateFixedValueMeasure(DatasourceFeign datasourceFeign) {
